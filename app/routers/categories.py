...
async def create_category(category: CategoryCreate,db: Session = Depends(get_db)):
    ...
    if category.parent_id is not None:
        ...
        if parent is None:
            raise HTTPException(status_code=400, detail="Родительская категория не найдена или не активна")
...
async def delete_category(category_id: int, db: Session = Depends(get_db)):
    """
    Удаляет категорию товара по ID.
    """
    stmt = select(CategoryModel).where(CategoryModel.id == category_id, CategoryModel.is_active == True)
    category = db.scalars(stmt).first()
    if category is None:
        raise HTTPException(status_code=404, detail="Категория не найдена или уже удалена")
